...
@EventBusSubscriber
public class SaferootModBiomes {
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.NETHER) || dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				if (levelStem.generator() instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((SaferootModNoiseGeneratorSettings) (Object) noiseGenerator.generatorSettings().value()).setsaferootDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	public static <T> Climate.ParameterList<T> adaptPresetParameterList(ResourceLocation idArg, Climate.ParameterList<T> originalList, Function<ResourceKey<Biome>, T> lookup) {
		...
		if (idArg.equals(OVERWORLD_BIOMESOURCE_PRESET_ID))
			return SaferootModBiomes.modifyOverworldParameterPoints(originalList, lookup);
		return originalList;
	}
	...
	private static SurfaceRules.RuleSource injectOverworldSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(
				preliminarySurfaceRule(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("saferoot", "rootia")), Blocks.ROOTED_DIRT.defaultBlockState(), Blocks.DIRT.defaultBlockState(), Blocks.GRAVEL.defaultBlockState()));
	...
	public static <T> Climate.ParameterList<T> modifyOverworldParameterPoints(Climate.ParameterList<T> originalList, Function<ResourceKey<Biome>, T> lookup) {
		...
		parameters.add(new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(-0.5f, 0.5f), Climate.Parameter.span(-0.5f, 0.5f), Climate.Parameter.span(0.3f, 1f), Climate.Parameter.span(-0.5f, 0.5f), Climate.Parameter.point(0.0f),
				Climate.Parameter.span(-1f, 1f), 0), lookup.apply(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("saferoot", "rootia")))));
	...
	public interface SaferootModNoiseGeneratorSettings {
		void setsaferootDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}