...
@EventBusSubscriber
public class RootiaTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(SaferootModBlocks.ROOTIA_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("saferoot:rootia_portal"), poiType);
	...
	public RootiaTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = SaferootModBlocks.ROOTIA_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
